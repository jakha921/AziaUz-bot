...
async def user_start(m: Message, db_session: AsyncSession, texts: Map):
    """User start command handler, ask contact if your contact have not on DB else show menu."""
    user_id = m.chat.id
    ...
    try:
        ...
        print(f'START : {err}')
        await m.answer(texts.user.press_start)
...
async def user_phone_sent(m: Message, texts: Map, db_user: TGUser, db_session: AsyncSession):
    ...
    try:
        ...
        if send_cont:
            ...
            await m.answer(f"Sorry, but {phone} number we can't registered ")
    except Exception as err:
...
async def user_get_token(m: Message, texts: Map, state: FSMContext, db_session: AsyncSession):
    """send confirm code to get token, if you are not verified go to state get_data """
    code = m.text
...
async def user_last_news(call: CallbackQuery, state: FSMContext, texts: Map):
    ...
    try:
        ...
        print(f'NEWS : {err}')
        await call.message.answer(texts.user.press_start)
...
async def user_balance(call: CallbackQuery, state: FSMContext, texts: Map):
    """Show user card and balance information if you had, else send msg you do not have cards."""
    token = await state.get_data("token")
    try:
...
async def user_lang_chosen(cb: CallbackQuery, callback_data: dict,
                           texts: Map, db_user: TGUser, db_session: AsyncSession):
    """User lang chosen handler"""
    code = callback_data.get('lang_code')
    await TGUser.update_user(db_session,
                             telegram_id=db_user.telegram_id,
                             updated_fields={'lang_code': code})

    # manually load translation for user with new lang code
...
async def user_occupation(call: CallbackQuery, db_session: AsyncSession, state: FSMContext, texts: Map):
    ...
    try:
        token = await state.get_data("token")
        print(post_user_details(token['token'], data))
        await state.finish()
...
def register_user(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(user_marital_status, text_contains="marital_status",
                                       state=UserDetails.marital_status)
    ...
    dp.register_callback_query_handler(user_lang_chosen, cd_choose_lang.filter(), state="*", )
